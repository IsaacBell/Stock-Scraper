...
private:
	...
	// Helpers
	static void subSelectRange(uint8_t n1, uint8_t n3);
};

double operator-(Date &a, Date&b) { return a -= b; }

uint8_t Date::__range = 0;
...
inline void Date::subSelectRange(uint8_t n1, uint8_t n3)
{
	if (n3 - n1 == 1) {
		Date::__range = (i_Range1[n1] < seconds) ? n1 : n3;
		return;
	}
	uint8_t n2 = (n1 +1+ n3) / 2;
